...
use business\core\UcaAuthenticator;

/**
 * Test authenticator adding mock identity data to a CAS-authenticated user.
 */
class ApplicationTestAuthenticator extends UcaAuthenticator
{
    /**
     * Performs CAS authentication and enriches the result with mock user data.
     *
    ...
    {
        $casResult = parent::authenticate();
        if ($casResult === false) {
            return false;
        }
        $casResult["name"] = "BOUHOURS Cédric";
        $casResult["status"] = "prof";
        return $casResult;
    }
}
